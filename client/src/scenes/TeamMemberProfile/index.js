import { Box, Typography, Avatar, Chip, Divider, CircularProgress, useTheme } from "@mui/material";
...
const TeamMemberProfile = () => {
    ...
    const selectedTeamMember = useSelector((state) => state.team.activeTeamMember);
    ...
    useEffect(() => {
      if (Object.keys(selectedTeamMember).length > 0) {
        setTeamMember({
          ...selectedTeamMember,
          dateOfJoining: format(selectedTeamMember.dateOfJoining, "yyyy-MM-dd"),
        });
      }
  ...
  return (
    ...
    >
      ...
      <Box width="100%" mb={2}>
        ...
        <Typography variant="body2" mb={1}>{teamMember.employmentType}</Typography>

        <Typography variant="subtitle1" fontWeight="bold">Date of Joining:</Typography>
        <Typography variant="body2" mb={1}>{new Date(teamMember.dateOfJoining).toLocaleDateString()}</Typography>

        <Typography variant="subtitle1" fontWeight="bold">Seniority Level:</Typography>
        <Typography variant="body2" mb={1}>{teamMember.seniorityLevel}</Typography>

        <Typography variant="subtitle1" fontWeight="bold">Remote Work Allowed:</Typography>
        <Typography variant="body2" mb={1}>{teamMember.remoteWorkAllowed ? "Yes" : "No"}</Typography>
      </Box>
      ...
      <Box width="100%" mb={2}>
        ...
        <Typography variant="body2">{teamMember.yearsOfExperience} year(s)</Typography>
      </Box>
    </Box>
  );
};
